...
<form method="post" action="options.php">
    ...
    <?php submit_button();    ?>
        ...
        <tr>
        	<td colspan="2">
        <strong>oAuth Bearer accesskey (passed in header when retrieving JSON):</strong>
        <br>
           <input type="text" name="jci_oauth_bearer_access_key" value="<?php echo get_option('jci_oauth_bearer_access_key'); ?>"/>
           </td>
        </tr>
        <tr>
        	<td colspan="2">
...
<a href="https://json-content-importer.com/?sc=wp" target="_blank"><img src="<?php echo plugins_url( 'images/icon-256x256.png', __FILE__ ); ?>" border="0" title="Learn more about JSON Content Importer PRO"></a>
		...
		<!--	<div class="rightsettings2">
					<h3>Save 10% Now!</h3>
					<p>Subscribe to the JCI-newsletter now:<br>We'll send you a 10%-discount code for your PRO-upgrade.</p>
<form action="https://json-content-importer.com/" id="mc4wp-form-1" class="mc4wp-form mc4wp-form-1053 mc4wp-form-submitted mc4wp-form-success" method="post" data-id="1053" data-name="Default sign-up form" >
<input required="required" id="s" placeholder="Your email address" name="EMAIL"  value="<?php echo (!empty($current_user->user_email) ?  $current_user->user_email : ''); ?>"  type="email">
<input type="submit" id="searchsubmit" class="prefix button" value="Sign up" />
...
<input type="hidden" name="_mc4wp_form_id" value="1053" />
